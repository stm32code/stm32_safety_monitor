...
unsigned char OneNet_FillBuf(char *buf)
{
	...
	{
		memset(text, 0, sizeof(text));
		sprintf(text, "\"hreat\":%d,", n_heart_rate);
		strcat(buf, text);
		
		memset(text, 0, sizeof(text));
		sprintf(text, "\"blood\":%d,", n_sp02); 
		strcat(buf, text);
	}
...
//有效数据
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"heart_v\":\"%d\",", threshold_value_init.heart_value); 
	strcat(buf, text);
	memset(text, 0, sizeof(text));
	sprintf(text, "\"temp_v\":\"%d\",", threshold_value_init.temp_value); 
	strcat(buf, text);
	memset(text, 0, sizeof(text));
	sprintf(text, "\"spo2_v\":\"%d\",", threshold_value_init.spo2_value); 
	strcat(buf, text);

	memset(text, 0, sizeof(text));
	sprintf(text, "\"wrring\":\"%d\"", device_state_init.Waning); // Temp是数据流的一个名称，temper是温度值
	strcat(buf, text);
